...
class modificaPaziente {
	...
	public function start() {
		...
		$sql = "SELECT idpaziente, cognome, nome, indirizzo, citta, cap, provincia, " .
				"eta, sesso, luogonascita, datanascita, codicefiscale, partitaiva, " .
				"telefonofisso, telefonoportatile, email, to_char(datainserimento, 'DD/MM/YYYY') as datainserimento, " .
				"to_char(datamodifica, 'DD/MM/YYYY') as datamodifica, idlistino, idmedico, " .
				"idlaboratorio " .
				"FROM paziente.paziente " .
				"WHERE idpaziente = " . trim($this->getIdPaziente());
	...
	private function modifica($paziente) {
		...
		$idPaziente = trim($paziente->getIdPaziente());
		...
		$sql =	"UPDATE paziente.paziente set " .
				"cognome='$cognome', nome='$nome', tipo='$tipo', indirizzo='$indirizzo', citta='$citta', " .
				"cap='$cap', provincia='$provincia', eta='$eta', sesso='$sesso', luogonascita='$luogoNascita', " .
				"datanascita='$dataNascita', codicefiscale='$codiceFiscale', partitaiva='$partitaIva', " .
				"telefonofisso='$telefonoFisso', telefonoportatile='$telefonoPortatile', email='$email', " .
				"datainserimento='$dataInserimento', datamodifica=current_date, idmedico=$medico, idlaboratorio=$laboratorio, idlistino=$listino " .
