...
class ricercaNotaTemplate extends preventivoAbstract {

	private static $filtri = "/preventivo/ricercaNota.filtri.html";
	private static $risultatiTesta = "/preventivo/ricercaNota.risultati.testata.html";
	private static $risultatiCorpo = "/preventivo/ricercaNota.risultati.corpo.html";
	private static $risultatiPiede = "/preventivo/ricercaNota.risultati.piede.html";
	...
	public function displayFiltri() {
		...
		$titoloPagina = ($_SESSION['idPreventivo'] != "") ? '%ml.ricercaNotaPreventivoPrincipale%' : '%ml.ricercaNotaPreventivoSecondario%'; 
	...
	public function displayRisultati() {
		...
		if ($numNote > 0) {
			...
			foreach(pg_fetch_all($_SESSION['notetrovate']) as $row) {
				 ...
				 */
				$bottoneModifica = "<a class='tooltip' href='../preventivo/modificaNotaPreventivoFacade.class.php?modo=start&idNotaPreventivo=" . $row['idnotapreventivo'] . "'><li class='ui-state-default ui-corner-all' title='Modifica'><span class='ui-icon ui-icon-pencil'></span></li></a>";
				$bottoneCancella = "<a class='tooltip' href='../preventivo/cancellaNotaPreventivoFacade.class.php?modo=start&idNotaPreventivo=" . $row['idnotapreventivo'] . "'><li class='ui-state-default ui-corner-all' title='Cancella'><span class='ui-icon ui-icon-trash'></span></li></a>";
				...
				$replace = array(
						'%class%' => $class,
						'%idnotapreventivo%' => $row['idnotapreventivo'],
						'%nota%' => $row['nota'],
