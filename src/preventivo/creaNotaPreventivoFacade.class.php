...
require_once 'creaNotaPreventivo.class.php';
...
if ($_POST['usa-sessione']) {
	...
	if ($firewall->controlloCampiRichiesta($data)) {
		...
		$creaNotaPreventivo = new creaNotaPreventivo();
		if ($_GET['modo'] == "start") $creaNotaPreventivo->start();
		if ($_GET['modo'] == "go") $creaNotaPreventivo->go();
	}
	else {
