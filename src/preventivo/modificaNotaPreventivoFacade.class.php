...
require_once 'modificaNotaPreventivo.class.php';
...
if ($_POST['usa-sessione']) {
	...
	if ($firewall->controlloCampiRichiesta($data)) {
		...
		$modificaNotaPreventivo = new modificaNotaPreventivo();
		if ($_GET['modo'] == "start") $modificaNotaPreventivo->start();
		if ($_GET['modo'] == "go") $modificaNotaPreventivo->go();
	}
	else {
